...
| 2026-08-07 | Confirmación de pago de Stripe (patrón redirect + verify, sin webhook): nuevo endpoint `POST /api/cart/checkout/stripe/confirm` que comprueba contra la API de Stripe que la sesión es del usuario y está pagada antes de crear el pedido. `POST /api/cart/checkout/stripe` ya no acepta los items por body — se toman siempre del carrito real del servidor, para que no se pueda manipular lo que se cobra. Los tests de integración pasan a usar emails propios (`integration-test-*`), distintos de los de `prisma/seed.js`, para no pisar las cuentas de prueba manual al ejecutar `npm test`. |
